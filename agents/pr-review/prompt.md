...
- `gh pr diff ${PR_NUMBER}` — the diff. This is the primary input. Files under `.github/agent-logs/` are agent execution logs auto-committed to PR branches — skip them, they are noise. Don't use `git diff` — the checkout is shallow (`fetch-depth: 1`) so comparisons against main have no merge base and will fail.
